/**
 * EntField -Functions for rendering Go struct field lines.
 *
 * Returns plain strings with proper newlines.
 * Called imperatively by EntStruct for synchronous rendering.
 */
...
export function generateFieldLine(
  ...
  imports: Set<string>,
  collectionStrategy?: EntEmitterOptions["collection-strategy"],
): string {
  ...
  if (isArrayType(prop.type)) {
    const { goType, entTag, requiredImports, doc } = resolveArrayGoType(
      program,
      prop,
      columnName,
      compositeMap,
    ...
    const structTag = buildStructTag(program, prop, entTag);
  ...
  if (enumInfo) {
    const { goType, entTag, doc } = resolveEnumGoType(
      program,
      prop,
      columnName,
      enumInfo,
  ...
  // Regular scalar type
  const { goType, entTag, requiredImports, doc } = resolveGoType(
    program,
    prop,
    columnName,
    compositeMap,
...
/**
 * Generate an ignored field (ent:"-") line.
 */
export function generateIgnoredFieldLine(
  ...
  return `${docComment}\t${fieldName} ${finalGoType} \`ent:"-" json:"${prop.name}${jsonOmit}"\`\n`;
}
...
function generateSoftDeleteFieldLine(
  ...
  return `${buildDocComment(doc)}\t${fieldName} ent.DeletedAt \`ent:"${tagParts.join(";")}" json:"${prop.name}"\`\n`;
}

function buildStructTag(program: Program, prop: ModelProperty, entTag: string): string {
  const validateTag = buildValidateTag(program, prop);
  const jsonOmit = prop.optional ? ",omitempty" : "";
  const jsonTag = `json:"${prop.name}${jsonOmit}"`;
  return validateTag
    ? `ent:"${entTag}" validate:"${validateTag}" ${jsonTag}`
    : `ent:"${entTag}" ${jsonTag}`;
}
...
function resolveGoType(
  ...
  let entTypeName = mapping?.entType ?? "";
  const maxLen = getMaxLength(program, prop);
  if (maxLen !== undefined && (dbType === "string" || entTypeName === "varchar(255)")) {
    entTypeName = `varchar(${maxLen})`;
  }
  const prec = getPrecision(program, prop);
  if (prec && (dbType === "decimal" || dbType === "float32" || dbType === "float64")) {
    entTypeName = `numeric(${prec.precision},${prec.scale})`;
  }
  if (entTypeName) tagParts.push(`type:${entTypeName}`);

  if (isKey(program, prop)) {
    tagParts.push("primaryKey");
    if (dbType === "uuid") {
      const customDefault = getDefaultValue(program, prop);
      tagParts.push(`default:${formatEntDefaultValue(customDefault ?? "gen_random_uuid()")}`);
    }
  }
  ...
  appendCommonEntTags(program, prop, columnName, compositeMap, tagParts);
  ...
  return {
    goType,
    entTag: deduplicateParts(tagParts).join(";"),
    requiredImports,
    doc,
  };
}
...
function resolveArrayGoType(
  ...
  if (!collectionStrategy) {
    ...
    entTypeName = "jsonb";
    requiredImports.push("entgo.io/ent/dialect/sql/schema");
  } else if (postgresElementType) {
    goType = `[]${elementGoType}`;
    entTypeName = `${postgresElementType}[]`;
  } else {
    reportDiagnostic(program, {
      code: "unsupported-type",
      format: {
        typeName: elementType ? describeTypeForDiagnostic(elementType) : "array",
        propName: prop.name,
      },
      target: prop,
    });
  ...
  if (entTypeName) {
    tagParts.push(`type:${entTypeName}`);
  }
...
function resolveEnumGoType(
  ...
  const numeric =
    enumInfo.members.length > 0 && enumInfo.members.every((m) => m.valueKind === "number");
  const maxValueLen = Math.max(...enumInfo.members.map((m) => m.value.length), 20);
  const entTypeName = numeric ? "integer" : `varchar(${maxValueLen})`;
...
function resolveArrayElementType(
  ...
  if (!dbType || !mapping) {
    reportDiagnostic(program, {
      code: "unsupported-type",
      format: {
        typeName: dbType ?? describeTypeForDiagnostic(elementType),
        propName: prop.name,
      },
      target: prop,
    });
...
function describeTypeForDiagnostic(type: Type): string {
  if (type.kind === "ModelProperty") {
    return describeTypeForDiagnostic(type.type);
  }
...
function appendCommonEntTags(
  ...
  if (!isKey(program, prop)) {
    ...
    if (defaultVal !== undefined) tagParts.push(`default:${formatEntDefaultValue(defaultVal)}`);
  }
...
function formatEntDefaultValue(value: string): string {
  return value === "" ? "''" : value;
}
