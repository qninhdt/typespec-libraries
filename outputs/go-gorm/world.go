...
type World struct {
	...
	CreatedAt time.Time `gorm:"column:created_at;type:timestamptz;autoCreateTime;not null;index:idx_worlds_owner_created,priority:2;comment:Timestamp when the record was created" json:"createdAt"`
	...
	Name string `gorm:"column:name;type:varchar(200);not null;index:idx_worlds_name;comment:Display name of the world - max 200 characters, indexed for lookup. Referenced via lookup type in derived models (e.g. 'worldName: World.name')." validate:"required,max=200" json:"name"`
	// System prompt used to guide AI generation within this world. Custom 'text' scalar - safe to target with @
	Prompt string `gorm:"column:prompt;type:text;not null;comment:System prompt used to guide AI generation within this world. Custom 'text' scalar - safe to target with @" validate:"required" json:"prompt"`
	// Visibility setting - controls who can discover and access this world
	Visibility WorldVisibility `gorm:"column:visibility;type:varchar(20);not null;index:idx_worlds_visibility;index:idx_worlds_visibility_active,priority:1;default:private;comment:Visibility setting - controls who can discover and access this world" validate:"oneof=private,unlisted,public" json:"visibility"`
	...
	Owner User `gorm:"foreignKey:OwnerID;constraint:OnDelete:CASCADE,OnUpdate:CASCADE" json:"owner,omitempty"`
	// Story tree nodes belonging to this world
	StoryNodes []StoryNode `gorm:"foreignKey:WorldID;constraint:OnDelete:CASCADE,OnUpdate:CASCADE" json:"storyNodes,omitempty"`
	// NPCs defined in this world
	Npcs []Npc `gorm:"foreignKey:WorldID;constraint:OnDelete:CASCADE,OnUpdate:CASCADE" json:"npcs,omitempty"`
	// Locations defined in this world
	Locations []Location `gorm:"foreignKey:WorldID;constraint:OnDelete:CASCADE,OnUpdate:CASCADE" json:"locations,omitempty"`
	// Events defined in this world
	GameEvents []GameEvent `gorm:"foreignKey:WorldID;constraint:OnDelete:CASCADE,OnUpdate:CASCADE" json:"gameEvents,omitempty"`
