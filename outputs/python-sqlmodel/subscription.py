...
class Subscription(SQLModel, table=True):
    ...
    plan: SubscriptionPlan = Field(sa_column=Column(SAEnum(SubscriptionPlan), nullable=False, index=True, comment="Subscription plan tier - determines access level and billing. Indexed for fast filtering by plan type."))
    ...
    monthly_price: Decimal = Field(sa_column=Column(Numeric(10, 2), nullable=False, comment="Monthly price stored as NUMERIC(10,2) to avoid floating-point rounding errors. Use @precision(10, 2) - do NOT use float or double for monetary values."))
    # Start date of the subscription period
    start_date: datetime = Field(sa_column=Column(DateTime(timezone=True), nullable=False, comment="Start date of the subscription period"))
