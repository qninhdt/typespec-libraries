...
class StoryNode(SQLModel, table=True):
    ...
    pool: dict[str, Any] = Field(
        sa_column=Column(
            JSONB,
            nullable=False,
            comment="Cumulative pool - NPC, tag, event, and location IDs from all ancestors",
        )
    )
    # Content introduced at this specific node
    new_content: dict[str, Any] = Field(
        sa_column=Column(
            JSONB, nullable=False, comment="Content introduced at this specific node"
        )
    )
    ...
    world_id: UUID = Field(
        ...
        sa_column_kwargs={
            "nullable": False,
            "comment": "Foreign key to World - required",
        },
    )
    # Foreign key to parent StoryNode - nullable for root nodes
    parent_id: UUID | None = Field(
        default=None,
        foreign_key="story_nodes.id",
        sa_column_kwargs={
            "comment": "Foreign key to parent StoryNode - nullable for root nodes"
        },
    )
